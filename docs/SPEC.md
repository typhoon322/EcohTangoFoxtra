...
| 回测数据 | `backend/backtest.db` | 价格历史数据库（**纳入 git，~5MB 种子数据，CI 直接 checkout**） |
...
| `backend/feishu_config.py` | 飞书混淆配置（LARK_PUSH_CFG） |
...
#   需在 repo Secrets 配置 LARK_PUSH_CFG（混淆 token，非明文 webhook）
#   本地: 复制 .env.example → .env，填入 LARK_PUSH_CFG
...
│   └── backtest.db               📦 SQLite 历史数据（git 跟踪，CI 种子）
├── frontend/
